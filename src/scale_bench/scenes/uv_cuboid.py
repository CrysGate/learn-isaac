...
def spawn_uv_cuboid(
    ...
    **kwargs,
    ...
    """Spawn an Isaac Lab cuboid, then add one UV island to each cube face."""

    if len(cfg.uv_scale) != 2 or any(scale <= 0.0 for scale in cfg.uv_scale):
        raise ValueError(f"UV scale must contain two positive values, got {cfg.uv_scale}.")

    # Delegate geometry, transforms, cloning, collision, and material setup to Isaac Lab.
...
class UvCuboidCfg(sim_utils.CuboidCfg):
    """Isaac Lab cuboid configuration extended with face-varying ``st`` UVs."""

    func: Callable = spawn_uv_cuboid

    uv_scale: tuple[float, float] = (1.0, 1.0)
    """Number of texture repeats in the U and V directions on each face."""
